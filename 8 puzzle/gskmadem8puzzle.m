clc;
pzstat = input('DarkArmy: Enter a puzzle in 3 x 3 form (use 0 for space) :   ');
%pzrstat = [1 2 3;4 5 6;7 8 0];
pzrstat = input('DarkArmy: Enter Goal State in 3 x 3 form (use 0 for space) :');
intv000 = size(pzstat);
if not(all(intv000 == [3 3]))
    msgbox('Input is not in correct format.............. The format must be 3 x 3 matrix. The blank space should be replaced by 0.','8-Puzzle DarkArmy');
else
    pzinl = [];
    pzhis = [];
    pzrhis = [];
